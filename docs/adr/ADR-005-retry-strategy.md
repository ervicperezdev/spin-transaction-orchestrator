...
La estrategia de retry debe distinguir entre clases de error en las que el retry es seguro, ambiguo o prohibido.
---
## Decisión
Adopte la siguiente política de retry y circuit breaker cuando Resilience4j (o un
...
**Política de retry: clasificación de error explícito:**
...
Retry de retroceso: exponencial con fluctuación, base 500 ms.
---
## Consecuencias
**Positivo:**
- El circuit breaker evita fallas en cascada: una vez que se determina que el proveedor no está en buen estado, llama a fast-fail inmediatamente en lugar de agotar el grupo de subprocesos.
- El no-retry explícito en el timeout obliga a abordar el diseño de reconciliación en lugar de ocultarlo detrás de retrys optimistas.
...
- El no retry explícito en el timeout significa que las llamadas agotadas aparecen como errores para la persona que llama. Los clientes deben manejarlos como estados potencialmente ambiguos y utilizar `Idempotency-Key` al reintentar.
